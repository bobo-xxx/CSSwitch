...
Plain launcher status reported Codex authentication, Gateway health, dynamic catalog, and Claude Science ready. Task 3 supplied the post-install sample that printed only the allowlisted Gateway proxy environment on loopback port `2999`. Task 4's second environment read was denied and was not retried or bypassed. No process was restarted or signalled. Across the permitted checks, the sampled Gateway and Science PIDs and listener ownership remained unchanged, and the sampled Gateway executable retained SHA-256 `300e65867fedd68ed42aa07839fad005d3e20c0f70827332a055ee31bf753d49`. No provider, model, credential, profile, fallback, Science data, tunnel, or firewall was changed. This proves Linux launcher restart configuration without performing a restart; live API-key failure induction and real macOS verification remain pending.
```
